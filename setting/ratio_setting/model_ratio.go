...
// getHardcodedCompletionModelRatio returns the built-in fallback completion
// ratio for a model. The boolean second return value historically meant
// "locked" (DB config ignored). It is now always false: the numeric value
// remains as a sane default when no `CompletionRatio` entry exists in the
// options table, but any operator-configured value (admin UI or models.dev
// sync) takes precedence. This keeps behavior of `CompletionRatio` consistent
// with `ModelRatio`, `ModelPrice`, and `CacheRatio`.
